...
type AgentWithRelations = UserAgent & {
  user: { piUsername: string | null; walletAddress: string; xp: number; stamps: { id: string }[] };
  ...
  <div class="card">
    <div class="header">
      ...
      <div>
        ...
        <div class="username">@${escapeHtml(agent.user.piUsername ?? 'unknown')}</div>
      </div>
    </div>
...
export async function publishAgentLandingPage(
  ...
  try {
    const agent = await prisma.userAgent.findUnique({
      where: { id: agentId },
      include: {
        user: { select: { piUsername: true, walletAddress: true, xp: true, stamps: { select: { id: true } } } },
        installedSkills: {
          where: { status: "active" },
          include: { skill: true },
          take: 20,
    ...
    const trustScore = calculateTrustScore(agent.user.xp, agent.user.stamps?.length ?? 0);
    ...
    logger.info(`[here.now] Published landing page for agent ${agent.user.piUsername ?? 'unknown'}: ${result.url}`);
