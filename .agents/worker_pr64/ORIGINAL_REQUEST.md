## 2026-06-18T20:12:47Z
Identity: You are the Verification Worker for PR 64.
Working directory: /Users/cryptojoker710/Desktop/AxiomID/.agents/worker_pr64/

Task:
1. Stash any local unstaged changes on the active branch to keep the repo clean.
2. Checkout the branch `optimize-kyaclaim-latency-3059620336710351602` (associated with PR 64). Note that this branch exists remotely as `origin/optimize-kyaclaim-latency-3059620336710351602`, so you may need to fetch and set up remote tracking.
3. Compare the branch to `main` (using git commands like `git diff main` or `git log main..optimize-kyaclaim-latency-3059620336710351602` or similar) to identify changed files and analyze design/complexity.
...
6. Post a review comment to GitHub PR 64 using `gh` CLI (e.g. `gh pr review 64 --comment -b "..."`). Make sure the review comment includes:
...
7. Write your detailed review and findings to `/Users/cryptojoker710/Desktop/AxiomID/.agents/worker_pr64/changes.md` and `/Users/cryptojoker710/Desktop/AxiomID/.agents/worker_pr64/handoff.md`.
